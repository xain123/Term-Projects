<?php
	...
	if(isset($_POST['submit']))
	{
		...
		{
			$errors["teacher"]="Teacher Does not Exist";
		}
		
		$queryerror3="select teacher_id from issue_return where teacher_id='$t_id' and book_id=$id";
		$res4=mysqli_query($connection,$queryerror3);
		$res5 = mysqli_fetch_assoc($res4);
		if(empty($res5["teacher_id"]))
		{
			$errors["teach"]="teacher_id and book_id does not match ";
		}
		
		if(empty($errors))
		{
		
			// $query1 ="select b.issue_date
								// from issue_date a, issue_return b
								// where a.issue_date = b.issue_date
								// and b.book_id = '$id'
								// and b.return_date is null;";
								
			$query1 ="select issue_date
								from  issue_return 
								where book_id = $id
								and return_date is null;";
			
			$result1 = mysqli_query($connection,$query1);	
			$issue_date = mysqli_fetch_assoc($result1);
			
			
			//$return_date=date("Y-m-d");
			$return_date=add_days(date("Y-m-d"),1);
			...
			
			{
				...
				if($stat["status_id"] == 1){
					...
					$query2 ="select a.due_date, b.issue_date
								from issue_date a, issue_return b
								where a.issue_date = b.issue_date
								and b.book_id = $id
								and b.return_date is null;";
...
<div class="main">
		...
		<div id="content">
			...
			<div class="issue">
					<form action="return_from_teacher.php" method ="post">
						Book Id:		<input   class="inputs"  type="number" name="book_id" min="1" value="<?php echo htmlspecialchars($id); ?>" /> <br />
