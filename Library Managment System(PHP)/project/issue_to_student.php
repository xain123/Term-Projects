<?php
	...
	if(isset($_POST['submit']))
	{
		...
		//$issue_date=add_days(date("Y-m-d"),1);
		...
		if(empty($errors))
		{
			...
			$query001="select student_id 
							from issue_return 
							where return_date is NULL 
							and student_id='$student_id'";
			
			$r3 = mysqli_query($connection,$query001);
			$student1 = mysqli_fetch_assoc($r3);
			
			if(empty($student1["student_id"])){
				...
				if($stat["status_id"] == 2){
					...
					$query1="insert into issue_return(book_id,issue_date,student_id,teacher_id,return_date,fine) 
							values ($id,'$issue_date','$student_id',null,null,null)";
				...
				{
					$errors["issue"]="Book is Already issued. ";
				}
			}
			else
			{
				$errors["return"]="Student should return previous issued book first";
...
<div class="main">
		...
		<div id="content">		
			...
			<div class="issue">
					<form action="issue_to_student.php" method ="post">
