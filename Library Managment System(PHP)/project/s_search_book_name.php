<?php
	...
	if(isset($_POST['submit']))
	{
		...
		$fields_required =array("book_name");
		...
		$fields_with_max_lengths = array("book_name" => 20);
		...
		$b_id = mysqli_fetch_assoc($result11);
		if(empty($b_id["book_id"]))
		{
...
<div class="main">
		...
		<div id="content">
			...
			<div class="issue">
					<form action="s_search_book_name.php" method ="post">
			
						Book Name:<select class="inputs"  type="text" name="book_name"  /> <br />
							<?php 
									
								$sql = "select book_name from isbn";
								$sql1=mysqli_query($connection,$sql);
								while ($disp = mysqli_fetch_assoc($sql1))
								{ 
									
									echo'<option value="'.$disp["book_name"].'">'.$disp["book_name"].'</option>';
